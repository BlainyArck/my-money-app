...
const login = (req, res, next) => {
    ...
    User.findOne({ email }, (err, user) => {
        if (err) {
            ...
            const token = jwt.sign(use, env.authSecret, {
                expiresIn: "1 day"
            })
            const { name, email } = user
            res.jason({ name, email, token })
        } else {
...
const signup = (req, res, next) => {
    ...
    if (!password.match(passwordRegex)) {
        return res.status(400).send({
            errors: [
                "Senha precisa ter: letra maiúscula, letra minúscula, número, caractere especial (@#$%) e tamanho entre 6 - 20."
            ]
        })
    }
